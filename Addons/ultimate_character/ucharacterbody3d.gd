...
var picked_object: Node
var has_grabbed : bool
var pull_power = 4 # the power of the forc pulling the picked object toward the hand position
...
func _enter_tree():
	...
	if !Engine.is_editor_hint():
		collision_shape_normal = $CollisionShapeNormal2
		collision_shape_crouch = $CollisionShapeCrouch
		head_node = $Head2
		camera_node = $Head2/Camera
		#raycast_node = $RayCast3D
		#Setting grabing nodes
		interaction = $Head2/Camera/Interaction
...
func _ready():
	...
	if Engine.is_editor_hint():
		...
		if nodes.size() > 0:
			collision_shape_normal = get_node("CollisionShapeNormal2")
			collision_shape_crouch = get_node("CollisionShapeCrouch")
			head_node = get_node("Head2")
	...
	if !Engine.is_editor_hint():
		...
		if Input.is_action_pressed(SPRINT):
			...
			if velocity.length() >= 0.1:
				camera_node.fov = lerpf(camera_node.fov, 90,  delta * 10.0)
			is_walking = false
			is_sprinting = true
			is_crouching = false
			...
			camera_node.fov = lerpf(camera_node.fov, 75,  delta * 10.0)
		...
		#Handle grabed objects
		var pick_collider = interaction.get_collider()
		
		# Toggle ouline around pickable objects
		if !has_grabbed:
			if pick_collider != last_grab_detected:
				if pick_collider != null && pick_collider.is_in_group("Pickable"):
					var pickable_mesh : MeshInstance3D = pick_collider.get_node("Outline")
					if pickable_mesh:
						pickable_mesh.visible = true
		...
		if picked_object != null:
			...
			elif picked_object is CharacterBody3D:
				picked_object.velocity.z = (b-a).z*pull_power
		last_grab_detected = pick_collider
		move_and_slide()
